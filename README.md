<div align="center">
  ...
  <h1>
  ONDEWO SIP Client Rust Library
  </h1>
</div>

This library gives a rust application typed, async access to the ONDEWO SIP
(SIP Telephony) gRPC API.

It is generated code plus a thin hand-written surface around it. The interface itself is defined
by the protocol buffer files of the [ONDEWO SIP API](https://github.com/ondewo/ondewo-sip-api),
...
The library is published to [crates.io](https://crates.io/crates/ondewo-sip-client) as
**`ondewo-sip-client`**, with the API documentation on
[docs.rs](https://docs.rs/ondewo-sip-client). Nothing else is needed to consume it - the stubs are
...
* Its version tracks the **ONDEWO SIP API** it was generated from in major and minor - a crate
...
```bash
git clone --recurse-submodules git@github.com:ondewo/ondewo-sip-client-rust.git
cd ondewo-sip-client-rust
...
module path mirrors the proto package: `package ondewo.sip;` is reachable as
`ondewo_sip_client::ondewo::sip`. Run `cargo doc --open` to browse the exact service
and message names of the API version this crate was generated from.

```rust
use ondewo_sip_client::ondewo::sip::*;
use tonic::transport::Channel;
use tonic::Request;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. A channel to the SIP server. `Channel` handles TLS, reconnects and
    //    connection pooling; the generated clients accept any tonic service.
    let channel = Channel::from_static("https://grpc-sip.ondewo.com:443")
        .connect()
        .await?;
    ...
    let token = std::env::var("ONDEWO_SIP_TOKEN")?;
...
```text
.
├── ondewo-sip-api                <----- submodule: the proto definitions to compile
...
1. `checkout_defined_submodule_versions` - checks out the pins at the top of the `Makefile`
   (`ONDEWO_SIP_API_GIT_BRANCH` and `ONDEWO_PROTO_COMPILER_GIT_BRANCH`)
1. `build_compiler` - builds `ondewo-rust-proto-compiler:latest` from the submodule
1. `update_cargo_version` - writes `ONDEWO_SIP_VERSION` into `Cargo.toml`
1. `generate_ondewo_protos` - runs the image over `ondewo-sip-api/ondewo` and writes `src/api`,
...
`SipClient`, so every declared RPC really is encoded, routed by its
`/ondewo.sip.Sip/<Method>` path, answered and decoded again. No ONDEWO server is involved.
...
Releases are cut from the `Makefile`. Bump `ONDEWO_SIP_VERSION`, add the matching
